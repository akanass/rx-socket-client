...
export class RxSocketClientTest {
    ...
    /**
     * Test if `webSocket` can receive data for given event emit by server
     */
    @test('- `webSocket` must receive Bytes data for given event emit by server')
    testRxSocketClientReceiveEmitBytes(done) {
        ...
        class SocketServerModule implements OnStart {
            ...
            onStart() {
                webSocket('ws://0.0.0.0:1235')
                    .onBytes('data', d => unit.string(Buffer.from(d).toString()).is('test message')
                        .when(() =>
                            Hapiness.shutdown().subscribe(_ => done())
                        )
                    );
    ...
    /**
     * Test if `RxSocketClient` can receive UTF message for given event send by server
     */
    @test('- `webSocket` must receive Bytes message for given event send by server')
